...
foreach ($categories as $category) {
    ...
    $range = 1;
    $countInFolder = 12;
    if (sizeof($vocabulary[$category]) > $countInFolder) {
        $range = ceil(sizeof($vocabulary[$category]) / $countInFolder);
    }
    foreach (range(1, $range) as $counter) {
        ...
        fwrite($fp, $data);*/
        //exit;
        foreach (range(1, $countInFolder) as $i) {
            ...
            for ($j = 1; $j <= 60; $j++) {
                //$background = imagecreatefrompng($backgroundFramesDirectroy . '/frame' . ($j) . '.png');
                //$modValue = $i % sizeof($backgrounds);
                //$backgroundImage = $modValue == 0 ? 1 : $modValue;  
                //$background = imagecreatefrompng('images/backgrounds/animated/' . $category . '/' . ($backgroundImage) . '.png');
                $background = imagecreatefrompng('images/thumbnailLarge.png');
                ...
                $dst_x = (imagesx($background) - $sx) - ((imagesx($background) - $sx) / 2);
                // Merge the stamp onto our photo with an opacity of 50%
