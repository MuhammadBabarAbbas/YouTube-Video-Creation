...
include ('includes/vocabulary_categories.php');
...
foreach ($categories as $category) {
    ...
    foreach (range(1, $range) as $counter) {
        ...
        foreach (range(2, 27) as $i) {
            ...
            $shape = imagecreatefrompng('images/snaps_categorized/' . $category . '/' . $actualword .
                '.png');
            ...
            if (!file_exists('images/SlideImages/' . $category)) {
                mkdir('images/SlideImages/' . $category, 0777, true);
            }

            // Save the image to file and free memory
            imagepng($background, 'images/SlideImages/' . $category . '/Slide' . $i . '.png');
        ...
        createAudioFile($category, $subscribeText, 28);
        ...
        shell_exec("ffmpeg -f concat -i videos/" . $category .
        ...
        $shape = imagecreatefrompng('images/snaps_categorized_small/' . $category . '/' .
            $firstWord . '.png');
        // Set the margins for the stamp and get the height/width of the stamp image
...
function createVideoFile($category, $wordText, $index)
{
    ...
    $imagePath = 'images/SlideImages/' . $category . '/Slide' . $index . '.png';
...
function createAudioFile($category, $text, $index)
{
    ...
    if (!is_dir("audios/"))
        ...
        if (substr(sprintf('%o', fileperms('audios/')), -4) != "0777")
            chmod("audios/", 0777);
    $marketingTextArray = array(0, 1, 28);
